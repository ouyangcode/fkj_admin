<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>今日访客管理</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <el-table :data="listdata" stripe border>
        ...
        <el-table-column prop="venddate" label="预计离开时间" width="160">
          <template slot-scope="scope">{{scope.row.vplanendtime | dateFormat}}</template>
        </el-table-column>
        <el-table-column prop="vendtime" label="实际离开时间" width="160">
          <template slot-scope="scope">{{scope.row.vendtime | dateFormat}}</template>
        </el-table-column>
        ...
        <el-table-column fixed="right" prop="vvisitorstatus" label="来访状态" width="160">
          <template slot-scope="scope">
            <el-tag type="">{{scope.row.vvisitorstatus == 0 ? '已来访登记' : '已离开登记'}}</el-tag>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    async getListData() {
      const { data: res } = await this.$http.get(`/visitInfo/list/${this.pagenum}/${this.pagesize}/1`)
