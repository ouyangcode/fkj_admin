    ...
    <el-card>
      ...
      <el-form v-model="searchForm" :inline="true" class="demo-form-inline">
        <el-form-item label="编号:">
          <el-input v-model="searchForm.uid" type="number" placeholder="请输入编号"></el-input>
        </el-form-item>
      ...
      <el-table :data="listdata" stripe border>
        ...
        <el-table-column fixed="right" label="操作" width="120">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="goUser(scope.row)">移至正常</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    async getListData() {
      const { data: res } = await this.$http.get(`/userInfo/list/${this.pagenum}/${this.pagesize}/1`)
    ...
    // 移至正常
    async goUser(row) {
      const confirmResult = await this.$confirm('是否移动到正常人员?', '提示', {
